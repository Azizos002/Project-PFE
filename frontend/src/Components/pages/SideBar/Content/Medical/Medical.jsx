...
const Medical = () => {
    ...
    const fetchIncomeData = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5000/api/medical/getMedical', {
                headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('http://localhost:5000/api/medical/createMedical', { description, amount, frequency }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5000/api/medical/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleSave = async (id) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:5000/api/medical/updateMedical/${id}`, {
    ...
    const calculateTotalMonthlyMedical = () => {
    ...
    const calculateTotalAnnualyMedical = () => {
        return incomeData.reduce((total, income) => {
            const amount = parseFloat(income.amount);
            switch (income.frequency) {
                case 'weekly':
    ...
    const totalMonthly = calculateTotalMonthlyMedical();
        ...
        <>
            ...
            <div className='PagesContent'>
                <div className="income-container">
                    <h2 className='TitlePages'>Medical</h2>
                    ...
                    <table className='income-table'>
                        ...
                        <tfoot className="customTableFooter">
                            <tr>
                                <td className="totalMonthly" colSpan="1">Total Medical Monthly</td>
                                <td className="totalAmount" colSpan="1">
                                    TND {calculateTotalMonthlyMedical().toFixed(2)}
                                </td>
                            </tr>
                            <tr>
                                <td className="totalMonthly" colSpan="1">Total Medical Annually</td>
                                <td className="totalAmount" colSpan="1">
                                    TND {calculateTotalAnnualyMedical().toFixed(2)}
                                </td>
                            </tr>
                        </tfoot>
