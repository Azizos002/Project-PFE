...
const Saving = () => {
    ...
    const fetchIncomeData = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:5000/api/saving/getSaving', {
                headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('http://localhost:5000/api/saving/createSaving', { description, amount, frequency }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5000/api/saving/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleSave = async (id) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:5000/api/saving/updateSaving/${id}`, {
    ...
    const calculateTotalMonthlySaving = () => {
    ...
    const calculateTotalAnnualySaving = () => {
        return incomeData.reduce((total, income) => {
            const amount = parseFloat(income.amount);
            switch (income.frequency) {
                case 'weekly':
    ...
    const totalMonthly = calculateTotalMonthlySaving();
        ...
        <>
            ...
            <div className='PagesContent'>
                <div className="income-container">
                    <h2 className='TitlePages'>Saving</h2>
                    ...
                    <table className='income-table'>
                        ...
                        <tfoot className="customTableFooter">
                            <tr>
                                <td className="totalMonthly" colSpan="1">Total Saving Monthly</td>
                                <td className="totalAmount" colSpan="1">
                                    TND {calculateTotalMonthlySaving().toFixed(2)}
                                </td>
                            </tr>
                            <tr>
                                <td className="totalMonthly" colSpan="1">Total Saving Annually</td>
                                <td className="totalAmount" colSpan="1">
                                    TND {calculateTotalAnnualySaving().toFixed(2)}
                                </td>
                            </tr>
                        </tfoot>
