...
const Tax = () => {
  ...
  const fetchIncomeData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/tax/getTax', {
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/tax/createTax', { description, amount, frequency }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSave = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/tax/updateTax/${id}`, {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/tax/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
    ...
    <>
      ...
      <div className='PagesContent'>
        ...
        <div className="income-container">
          <h2 className='TitlePages'>Tax & Debt Payments</h2>
